...
async function apiPriceBars(symbol, timeframe = '5Min', limit = 100) {
  return _get(`/api/prices/bars?symbol=${encodeURIComponent(symbol)}&timeframe=${timeframe}&limit=${limit}`)
}
async function apiCalendarEvents()    { return _get('/api/calendar/events') }
async function apiWatchlistGet()      { return _get('/api/watchlist/') }
async function apiWatchlistAdd(ticker){ return _post('/api/watchlist/', { ticker }) }
async function apiWatchlistRemove(ticker) { return _delete('/api/watchlist/' + encodeURIComponent(ticker)) }
async function apiAiPremarketBrief()  { return _get('/api/ai/premarket-brief') }
